...
@section('content')
    ...
    <form action="{{ route('item-in.store') }}" method="POST">
        @csrf
        <div class="py-4 flex flex-col gap-4">
            <div>
                <p class="opacity-75 flex text-xl font-bold">Tanggal Masuk</p>
                <input type="date" class="text-xl p-2 rounded-lg border border-primary-dark w-1/4" name="in_date" value="{{date('Y-m-d')}}">
            </div>
            <div>
                <p class="opacity-75 flex text-xl font-bold">Kuantitas</p>
                <input type="text" class="text-xl p-2 rounded-lg border border-primary-dark w-1/4" name="in_quantity">
            </div>
            <div>
